...
//////////////////////////////////////////////
            ...
            if($.session.get('utm_source') == "undefined"){
              $.session.set('utm_source', ''+GETArr.utm_source+'');
            }
            if($.session.get('utm_medium') == "undefined"){
              $.session.set('utm_medium', ''+GETArr.utm_source+'');
            }


            //$('span.utm_medium input').val('test');

            var answ = $.session.get('utm_source');
            var answ2 = $.session.get('utm_medium');
            console.log("utm_source = "+answ);
            console.log("utm_medium = "+answ2);

            $('.sendNow').click(function(){
              setTimeout(function(){
                //$('span.utm_medium input').val(''+$.session.get('utm_medium')+'');
                //$('span.utm_source input').val(''+$.session.get('utm_source')+'');
                console.log($.session.get('utm_source'));
                if($.session.get('utm_source') != "undefined") $('input[name=utm_source]').val($.session.get('utm_source'));
                if($.session.get('utm_medium') != "undefined") $('input[name=utm_medium]').val($.session.get('utm_medium'));
